...
export default (): React.ReactNode => {
  ...
  return (
    <PageHeaderWrapper>
      <Card>
        <Printer>
          <SchemaForm components={{ Input }} actions={actions} onSubmit={(v) => console.log(v)}>
            <FormTab name="tabs" defaultActiveKey={'tab-2'}>
              <FormTab.TabPane name="tab-1" tab="选项1">
                <Field type="string" name="a1" title="字段1" required x-component="Input" />
              </FormTab.TabPane>
              <FormTab.TabPane name="tab-2" tab="选项2">
                <Field type="string" name="a2" title="字段2" required x-component="Input" />
                <Field type="string" name="a3" title="字段3" required x-component="Input" />
                <Field type="string" name="a4" title="字段4" required x-component="Input" />
                <Field type="string" name="a5" title="字段5" required x-component="Input" />
              </FormTab.TabPane>
            </FormTab>
            <FormButtonGroup>
              <Submit />
      ...
      <Card title={formatMessage({ id: 'formily.demo.intro' })}>
        <Paragraph>
          <ul className="react-demo-ul">
            <li className="react-demo-li">從 @formily/antd-components 中導出 FormTab</li>
            <li className="react-demo-li">
              {' '}
              FormTab 中的渲染是會強制全部渲染的，主要是為了收集校驗
            </li>
            <li className="react-demo-li">
              如果被隱藏的 Tab 校驗錯誤，在 Tab Title 上會展現 Badge 小紅標，同時瀏覽器自動滾動
            </li>
          </ul>
        </Paragraph>
      </Card>
